...
- Evaluated via a trained **XGBoost Classifier** yielding `sequence_score ∈ [0, 1]`.
...
- Inference: Evaluates 1–2 hop ego-subgraphs dynamically, outputting `network_score ∈ [0, 1]` and top contributing neighbor edges.
...
- Unsupervised **Isolation Forest** fitted on legitimate baseline behavior, outputting `anomaly_score ∈ [0, 1]`.

### 2.5 Typology & Motif Detector
- Deterministic rules identifying complex money laundering topographies:
  - **Rapid Pass-Through:** Forwarding $\ge 80\%$ of received funds within tight time windows.
  ...
  - **Circular Routing:** Multi-hop closed-loop cycles ($A \to B \to C \to A$).
